...
anova(pherbase.gls, pherbase1.lme, pherbase2.lme, pherbase5.lme) #best is pherbase5.lme but choose pherbase2.lme
...
ggplot(data=pherbase.sum, aes(x=T, y=CellsBase, shape=treatment, color=treatment)) + geom_point(size=5)+ 
  geom_errorbar(aes(ymin=CellsBase-sd, ymax=CellsBase+sd), width=15, size=1) + 
